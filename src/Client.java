...
public class Client extends RSApplet {
	 ...
	 */
	public static String clientName = "Dank Meme's Melting Gay Marijuana V0.1";
	...
	public void drawChannelButtons() {
		...
		case 5:
		case 6:
			cacheSprite[16].drawSprite(channelButtonsX[cButtonCPos], 143);
			break;
		}
		if (cButtonHPos == cButtonCPos) {
			...
			case 6:
			case 7:
				cacheSprite[17].drawSprite(channelButtonsX[cButtonHPos], 143);
				break;
			}
		} else {
			switch (cButtonHPos) {
			...
			case 5:
			case 6:
				cacheSprite[15].drawSprite(channelButtonsX[cButtonHPos], 143);
				break;
			case 7:
				cacheSprite[18].drawSprite(channelButtonsX[cButtonHPos], 143);
				break;
			}
		}
		int[] modes = { publicChatMode, privateChatMode, clanChatMode, tradeMode, duelMode };
		for (int i = 0; i < modeNamesX.length; i++) {
			smallText.method389(true, modeNamesX[i], 0xffffff, modeNames[i], modeNamesY[i]);
		}
		for (int i = 0; i < modeX.length; i++) {
			smallText.method382(textColor[modes[i]], modeX[i], text[modes[i]], 164, true);
		}
	}
	...
	private final void minimapHovers() {
		...
		prayHover = super.mouseX >= 525 && super.mouseX <= 581 && super.mouseY >= 105 && super.mouseY < 139;
		runHover = super.mouseX >= 584 && super.mouseX <= 613 && super.mouseY >= 132 && super.mouseY < 165 
			|| super.mouseX >= 557 && super.mouseX <= 584 && super.mouseY >= 142 && super.mouseY < 165;
	...
	private void processTabClick() {
		...
		if (super.clickMode3 == 1) {
			for (int i = 0; i < tabClickX.length; i++) {
				if (super.mouseX >= tabClickStart[i] && super.mouseX <= tabClickStart[i] + tabClickX[i] && super.mouseY >= tabClickY[i] && super.mouseY < tabClickY[i] + 37 && tabInterfaceIDs[i] != -1) {
					tabID = i;
					tabAreaAltered = true;
					break;
				}
	...
	private void drawMinimap() {
		...
		cacheSprite[19].drawSprite(0, 0);
		cacheSprite[22].drawHoverSprite(1, 46, 516, 0, cacheSprite[23]);
		loadAllOrbs();
		if (menuOpen)
			drawMenu(516, 0);
		aRSImageProducer_1165.initDrawingArea();
	...
	private void processMinimapActions() {
		...
		if (counterHover) {
			menuActionName[2] = counterOn ? "Toggle XP Display" : "Toggle XP Display";
			menuActionID[2] = 474;
		...
		if (prayHover) {
			menuActionName[2] = prayClicked ? "Turn Quick-" + prayerBook + " off" : "Turn Quick-" + prayerBook + " on";
			menuActionID[2] = 1500;
			menuActionRow = 2;
